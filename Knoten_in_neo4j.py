...
def get_definition(cui, term):
    ...
    return askOllama("Explain in two short sentences the meaning  in healthcare of the following term in:", term)
...
def askOllama(task, term, model='dolphin-phi',version= "1.0", temperature=0.1, role='assistant'):
    response = ollama.chat(model=model, messages=[{
        'role': role, # Rolle ist assistant zu sein
    ...
    result = response['message']['content'].strip() # Ergebnisse extrahieren
    ...
    if "synonyms" in task.lower():
        ...
        return [f"{syn.strip()} (generated with {model}, version {version}, at {timestamp})" for syn in result.split(",")]

    # Wenn in Aufgabe Wörter wie "explain" enthalten ist
